...
def baixar_dados_historicos(modo: str, ano: int):
    setup_logging()
    ...
    if not os.path.exists(parquet_path):
        logging.info(f"[HISTÓRICO] Processando {modo} {ano}")
        download_csv(url, csv_path)
        convert_csv_to_parquet(csv_path, parquet_path)
    else:
        logging.info(f"[HISTÓRICO] {parquet_path} já existe. Pulando...")


if __name__ == "__main__":
    for modo in ["EXP", "IMP"]:
        baixar_dados_historicos(modo)
